...
一个运行在个人 Windows 电脑上的局域网笔记网站。文档直接保存为普通 Markdown 文件，支持多级目录、所见即所得编辑、自动保存、搜索、最近文档、回收站、附件，以及当前文档的 Markdown、Word 和 PDF 导出。
...
- 打开文档后，右上角“导出”可选择 Markdown、Word 或 PDF。
- 导出直接读取编辑器中的最新标题和正文，即使内容仍在等待自动保存也会包含在文件中。
- Word/PDF 会保留标题、列表、引用、代码块、表格、文字颜色、链接和本地图片；缺失或损坏图片会显示替代文字。
- PDF 中文输出使用 Windows 系统字体，默认查找微软雅黑、等线和宋体。

## 安全与备份

本项目没有登录和 HTTPS，只应运行在可信局域网。任何能访问端口的设备都可以编辑或删除笔记。

项目不内置全量备份。当前文档导出不能替代数据备份，建议定期把整个 `DATA_DIR` 复制到其他磁盘。
